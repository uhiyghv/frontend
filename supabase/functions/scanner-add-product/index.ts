...
const cleanCategory = (cat: string): string => cat.replace(/^[a-z]{2}:/, "").trim();
  ...
  try {
    ...
    return {
      ...
      nutritional_values: p.nutriments ? {
        ...
        saturatedFat: p.nutriments["saturated-fat_100g"] ?? p.nutriments.saturated_fat ?? null,
      ...
      carbon_footprint: p.ecoscore_data?.agribalyse ? {
        ...
        transportation: p.ecoscore_data.agribalyse.co2_transportation ?? null,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!scanner || scanner.user_id !== user.id) return jsonResponse({ error: 'Scanner non autorizzato' }, 403)
